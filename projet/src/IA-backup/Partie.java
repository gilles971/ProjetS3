...
*/
	public Partie(Joueur j) {
		...
		this.ia = new IA(monde.getLongueurCote(), monde.getLongueurCote(), monde.getNbFleches());
		this.nbrTour=0;						//Modifié pour S3
		this.listeInfo=new ArrayList<String>();
	}	
 ...
 */
	...
	*/
	...
	/*	Case[][] c = this.monde.getPlateau();		// #debug
		...
		boolean fait = false;
		String com = "";
		//Scanner sc = new Scanner(System.in);					//Modif pour S3
		...
		{			
			while(!fait)
			{				
				//sc.nextLine();									//Modif pour S3
				
				System.out.println("Choisissez une commande :\n");
				
				//sc.reset();
				//com = sc.nextLine();								//Modif pour S3
				com = this.ia.jouer(this.joueur.getCoordX(), this.joueur.getCoordY(), this.listeInfo );
